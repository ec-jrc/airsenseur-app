...
.factory('$AirSensEUR', function($http, $q, $networksSources, CONFIG){
  ...
  obj.getAllStations = function(){
    ...
    angular.forEach(arraySources, function(source, key){
      var service = "";
	  //source.APIurl = source.APIurl.replace("http://","https://");
      //console.log(source);
      ...
      if (source.APIurl != "https://aqsens.52north.org/data/reference/api/") {
        ...
        arrayPromise[source.APIurl] = $http.get(source.APIurl+"stations"+service, {cache: true, timeout: 60500});
         ...
         arrayPromise[source.APIurl] = $http.get(source.APIurl+"features"+service, {cache: true, timeout: 60500});
      }
    });
    ...
    $q.all(arrayPromise).then(function(success){
      ...
      angular.forEach(success, function(value, key){
          console.log("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-");
          var modArray = value.data;
          angular.forEach(modArray, function(value, key) {
    ...
    angular.forEach(arraySources, function(source, key){
      ...
      if (idPhenomenon !== "" ){
        ...
        if (source.APIurl != "https://aqsens.52north.org/data/reference/api/") {
            ...
            arrayPromise[source.APIurl] = $http.get(source.APIurl+"features/?phenomena="+idPhenomenon+service, {cache: true});
        }
      }
  ...
  obj.getTimeSeries = function(idStation, idphenomenon, networkSource){
    ...
    console.log("!!!!!!!!!!!!!!!!!!!!!!!!!");
    if (networkSource != "https://aqsens.52north.org/data/reference/api/") {
        ...
        console.log('getTimeSeries : ', networkSource+"datasets/?expanded=true&force_latest_values=true&features="+idStation+"&phenomena="+idphenomenon+service);
        $http.get(networkSource+"datasets/?expanded=true&force_latest_values=true&features="+idStation+"&phenomena="+idphenomenon+service, {cache: true}).then(
          function(success){
            console.log('HTTP REQUEST: ', networkSource+"datasets/?expanded=true&features="+idStation+"&phenomena="+idphenomenon+service);
            def.resolve(success.data);
            //console.log(success);
          },
          function(error){
  ...
  obj.getTimeSeriesData = function(idTimeseries, timespan, networkSource){
    ...
    if (networkSource != "https://aqsens.52north.org/data/reference/api/") {
        ...
        console.log('getTimeSeriesData : ', networkSource+"datasets/"+idTimeseries+"/observations?timespan="+timespan+"&generalize=true&generalizing_algorithm=lttb&treshold=50&expanded=true&format=flot"+service);
        $http.get(networkSource+"datasets/"+idTimeseries+"/observations?timespan="+timespan+"&generalize=true&generalizing_algorithm=lttb&treshold=50&expanded=true&format=flot"+service, {cache: true}).then(
          function(success){
            def.resolve(success.data);
            console.log(success); //return an array, we presume it will alaways be an array with 1 timesteries?
          },
...
.factory('$database', function($q){
  ...
  obj.getAllMonitoringNetworks = function(){
    ...
    obj.db.executeSql(query, [], function(success){
      ...
      if(success.rows.length > 0){
          for(var i = 0; i < success.rows.length; i++){
			//console.log(success.rows.item(i));
			var APIurl = success.rows.item(i).APIurl;
			console.log(APIurl);
			//APIurl = APIurl.replace("http://", "https://");
			var newItem = success.rows.item(i);
			newItem.APIurl = APIurl;
            //monitoringNetworks.push(success.rows.item(i));
            monitoringNetworks.push(newItem);
          }
  ...
  obj.getAllSelectedMonitoringNetworks = function(){
    ...
    obj.db.executeSql(query, [], function(success){
      ...
      if(success.rows.length > 0){
          for(var i = 0; i < success.rows.length; i++){
			//if (i > 0) {  // Exclude the JRC service because does not work - 404 error
				var newSource = success.rows.item(i);
				var APIurl = newSource.APIurl;
				newSource.APIurl = APIurl;
	            monitoringNetworks.push(newSource);
			//}
          }
        def.resolve(monitoringNetworks);
      }else{
        ...
        function(localLastUpdate){
          ...
          obj.getJsonServer('update.json').then(
            function(serverLastUpdate){
			  console.log("Server Last Update: " + serverLastUpdate.lastUpdate);
			  console.log("Local Last Update: " + localLastUpdate.timestamp);
...
.factory('$alerts', function($rootScope, $http, $q, $geolocationFactory, $AirSensEUR, $networksSources, CONFIG){
  ...
  obj.retrieveConcentrationNearUserForAllPollutants = function(){
	  if (typeof $networksSources.sources !== 'undefined') {
		  if (typeof $networksSources.sources[0] !== 'undefined') {
